    ...
    for mode in modes:
        for crdt in crdts:
            for operation in operations:
                p = pathlib.Path(f"gnuplot/results/{mode}/{crdt}")
...
def parse_files(path: str) -> Dict:
    ...
    # print(data_frame)
    print(data_frame[["size", "crdt", "encryption", "crdt_op", "Average Response Time", "Requests/s"]].to_string(
        index=False))
    data_frame[["size", "crdt", "encryption", "crdt_op", "Average Response Time", "Requests/s"]].to_excel(
        'results_set_update.xlsx', index=False, header=True)
    data_frame.to_excel('results_set_update_full.xlsx', index=False, header=True)
...
if __name__ == '__main__':
    # parse_files("set_results_update")
    #parse_files("set_results_1_user")
    parse_files("set_results_1_user_query")
