...
def parse_files(path: str) -> Dict:
    ...
    # print(data_frame)
    print(data_frame[["nclients", "crdt", "encryption", "crdt_op", "set_size", "Average Response Time", "Requests/s"]].to_string(
        index=False))
    data_frame[["nclients", "crdt", "encryption", "crdt_op", "set_size", "Average Response Time", "Requests/s"]].to_excel(
        'results_set.xlsx', index=False, header=True)
    data_frame.to_excel('results_set_full.xlsx', index=False, header=True)
